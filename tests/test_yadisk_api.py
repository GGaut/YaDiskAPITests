...
class Test_YaDisk_Folders:
    ...
    def test_create_folder(self, folder_manager):
        ...
        data = response.json()
        val_data = response_validation(data, CreatedResponse)

        assert "method" in val_data and val_data["method"] is not None, (
            "Error response body doesn't contain 'method' field"
        )
        assert "href" in val_data and val_data["href"] is not None, (
            "Error response body doesn't contain 'href' field"
        )
        assert "templated" in val_data and val_data["templated"] is not None, (
            "Error response body doesn't contain 'templated' field"
        )
